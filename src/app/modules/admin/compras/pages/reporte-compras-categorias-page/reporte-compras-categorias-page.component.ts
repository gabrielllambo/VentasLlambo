...
@Component({
    selector: 'app-reporte-compras-categorias-page',
    templateUrl: './reporte-compras-categorias-page.component.html',
    styleUrl: './reporte-compras-categorias-page.component.scss',
})
export class ReporteComprasCategoriasPageComponent implements OnInit, OnDestroy {
    ...
    getFilterComboConsulta() {
        ...
        forkJoin({
            ...
            next: (response) => {
                ...
                this.filtroAnalisisComprasForm.get('categoria')?.setValue(this.allCategoriaDataSource);
            },
            error: () => {
    ...
    getResumen(hideFilter: boolean) {
        ...
        this._detalleComprasService.GetAnalisisCategoriasByFilterAsync(request).subscribe((response: CompraAnalisisCategoriasDTO) => {
            ...
            this.hideSkeleton();
            this.disabledExportar = response.distribucionComprasCategoria.totalComprasCategorias.length > Numeracion.Cero ? Flags.False : Flags.True;
        }, () => {
            this._toolService.showError(DictionaryErrors.Transaction, DictionaryErrors.Tittle);
            this.disabledBuscar = Flags.False;
            if (hideFilter) { this.closedDrawer(); }
    ...
    generateCharts() {
        ...
        this.generarChartEvolucionCategoriaFecha();
        this.generarChartTotalizadoCategorias();
        this.generarChartTopDiezCategorias();
        this.generarChartEvolucionFecha();
    }

    generarChartEvolucionCategoriaFecha() {
        const seriesData = this.comprasAnalisisDataSource.lstEvolucionComprasCategoriaFecha.map(categoria => ({
            color: categoria.colorCategoria,
            name: categoria.nombreCategoria,
            data: this.rangoFecha.map(fecha => {
                const encontrado = categoria.datosComprasAgrupados.find(g => this._toolService.formatoFecha(g.fechaCompra) === fecha);
                return encontrado ? encontrado.montoCompraTotal : Numeracion.Cero;
            }),
        }));

        this.chartEvolucionComprasCategoriaFecha = {
            series: seriesData,
    ...
    generarChartTotalizadoCategorias() {
        const isMovilSize = this.isMobilSize();
        const total = this.comprasAnalisisDataSource.distribucionComprasCategoria.totalComprasCategorias.reduce((a, b) => a + b, 0);
        ...
        this.chartTotalizadoCategoria = {
            ...
            labels: this.comprasAnalisisDataSource.distribucionComprasCategoria.nombreCategorias,
            series: this.comprasAnalisisDataSource.distribucionComprasCategoria.totalComprasCategorias,
            noData: { text: 'Sin datos en el período', align: 'center', verticalAlign: 'middle', style: { color: '#000000', fontSize: '14px' } },
            colors: this.comprasAnalisisDataSource.distribucionComprasCategoria.coloresCategorias,
            tooltip: { enabled: true, y: { formatter: (val: number) => this.currencyNumberFormat.format(val) } },
    ...
    generarChartTopDiezCategorias() {
        ...
        this.chartTopDiezCategorias = {
            series: [{ name: 'Total', data: this.comprasAnalisisDataSource.topDiezCategoriasCompras.totalMontos }],
            noData: { text: 'Sin datos en el período', align: 'center', verticalAlign: 'middle', style: { color: '#000000', fontSize: '14px' } },
            colors: this.comprasAnalisisDataSource.topDiezCategoriasCompras.colores,
            ...
            xaxis: { categories: this.comprasAnalisisDataSource.topDiezCategoriasCompras.categorias, labels: { show: false } },
        };
    }

    generarChartEvolucionFecha() {
        const currencyFormat = this.currencyNumberFormat;
        this.chartEvolucionComprasFecha = {
            series: [{ color: '#7C3AED', name: 'Total', data: this.comprasAnalisisDataSource.evolucionComprasFecha.totalCompra }],
            noData: { text: 'Sin datos en el período', align: 'center', verticalAlign: 'middle', style: { color: '#000000', fontSize: '14px' } },
            chart: { height: 350, type: 'line', zoom: { enabled: false }, toolbar: { show: false } },
            dataLabels: { enabled: true, textAnchor: 'start', formatter: (val: number) => currencyFormat.format(val), offsetX: 0, dropShadow: { enabled: true } },
    ...
    getReportePorCategoriaAsync() {
        if (!this.comprasAnalisisDataSource?.distribucionComprasCategoria?.totalComprasCategorias?.length) return;
        ...
        this._detalleComprasService.GetReportePorCategoriasAsync(request).subscribe((response: any) => {
            ...
            if (response) {
                ...
                a.download = `Reporte_Compra_Por_Categorias_${this.getFechaFiltroCadenaReporte()}.xlsx`;
    ...
    obtenerRequest(): ObtenerReporteCategoriaCompraRequest {
        const request = new ObtenerReporteCategoriaCompraRequest();
        ...
        if (this.selectCategorias) {
            this.selectCategorias.options.filter(x => x.selected && x.value != 0).forEach(x => request.lstCategorias.push(x.value.id));
        }
    ...
    obtenerRequestReporte(): ObtenerReporteCategoriaCompraRequest {
        const request = new ObtenerReporteCategoriaCompraRequest();
        request.destinationTimeZoneId = this._toolService.getTimeZone();
        request.idUsuario = this.decodeToken.idUsuario;
        request.fechaCompraInicio = this.filtroAnalisisComprasForm.get('fechaCompraInicio')?.value ?? null;
